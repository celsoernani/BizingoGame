...
export default function Game({ player, initialPieces, initialTriangles }) {
  ...
  useEffect(() => {
    socket.on('players', ({ players }) => {
      ...
      if (players.length > 1) {
        ...
        toast.info(
          `O jogador ${players[indexPlayer0].name} quem começa o jogo.`
        );
      }
      setPlayers(players);
    });
  ...
  const checkKill = (newTriangle, piecesAux, pieceIndex) => {
    ...
    const triangleDown = trianglesAux.find(
      t => t.top === newTriangle.top + 45 && newTriangle.left === t.left
    );
    const triangleRight = trianglesAux.find(
      t => t.left === newTriangle.left - 50 && newTriangle.top === t.top
    );
    const triangleLeft = trianglesAux.find(
      t => t.left === newTriangle.left + 50 && newTriangle.top === t.top
    );
  ...
  const checkKillAll = () => {
    ...
    stateGame.triangles.forEach(triangle => {
      if (triangle.piece) {
        const triangleDown = stateGame.triangles.find(
          t => t.top === triangle.top + 45 && triangle.left === t.left
        );
        const triangleRight = stateGame.triangles.find(
          t => t.left === triangle.left - 50 && triangle.top === t.top
        );
        const triangleLeft = stateGame.triangles.find(
          t => t.left === triangle.left + 50 && triangle.top === t.top
        );
        if (triangleDown && triangleRight && triangleLeft) {
          if (triangleDown.piece && triangleRight.piece && triangleLeft.piece) {
            const killPiece = stateGame.pieces.find(
              p => p.labelPosition === triangle.label
            );
            if (killPiece) {
              ...
              setstateGame({
                ...stateGame,
                pieces: [...auxPieces, killPiece],
                triangles: [...auxTriangles, triangle],
              });
  ...
  const removePieceInTriangle = (triangleWithPiece, positionPieceOld) => {
    ...
    const triangleIndex = stateGame.triangles.findIndex(
      t => t.label === triangleWithPiece.label
    );
    const trianglesAux = stateGame.triangles;
  ...
  const prepMove = (positionTriangle, positionPiece) => {
    ...
    if (
      player.side === players[activatePlayer].side &&
      players[activatePlayer].turn
    ) {
      if (positionPiece) {
        ...
        if ('id' in selectedPiece) {
          if (selectedPiece.side === positionTriangle.side) {
            ...
            toast.error(
              'Você só pode se movimentar para triangulos da mesma cor.'
            );
          }
          socket.emit('changeGame', { stateGame });
        } else {
  ...
  return (
    <div style={{ flex: 1, flexDirection: 'column' }}>
      ...
      {stateGame.triangles.length > 0 &&
        stateGame.pieces.length > 0 &&
        players.length > 1 && (
          <Board
            triangles={stateGame.triangles}
